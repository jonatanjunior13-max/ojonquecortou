...
foreach ($key in $envs.Keys) {
    $val = $envs[$key]
    Write-Host "Adding $key to Vercel production..." -ForegroundColor Cyan
    npx vercel env add $key production --value $val --yes --force
    
    Write-Host "Adding $key to Vercel preview..." -ForegroundColor Cyan
    npx vercel env add $key preview --value $val --yes --force
    
    Write-Host "Adding $key to Vercel development..." -ForegroundColor Cyan
    npx vercel env add $key development --value $val --yes --force
}

Write-Host "All environment variables added! Triggering redeploy..." -ForegroundColor Green
npx vercel --prod --yes
