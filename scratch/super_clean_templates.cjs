...
function superCleanStyleAndData(filePath) {
  ...
  // 2. Clean style="..." attributes, resolving mismatched quotes
  const styleRegex = /style="([^"]*)"/g;
  
  html = html.replace(styleRegex, (match, styleContent) => {
    // Replace &quot; with BAD_QUOTE to prevent splitting by semicolon inside the entity
    ...
    const cleanedProps = props.filter(prop => {
      ...
      // Filter out properties with unclosed BAD_QUOTE or corrupted quotes
...
async function run() {
  console.log("Starting super clean process for templates...");
  ...
  try {
    ...
    if (Object.keys(updates).length > 0) {
      ...
      console.log("-> Firestore settings/studio document successfully updated with super-cleaned templates!");
    } else {
