...
function seoLinksPlugin() {
  return {
    ...
    transformIndexHtml(html) {
      const blogLinks = posts.map(post => `<a href="/blog/${post.slug}">${post.title}</a>`).join('\n        ');
      const serviceLinks = SEED_SERVICES.map(s => `<a href="/servicos/${s.id}">${s.name}</a>`).join('\n        ');
      const injection = `<nav style="position: absolute; width: 1px; height: 1px; padding: 0; margin: -1px; overflow: hidden; clip: rect(0, 0, 0, 0); white-space: nowrap; border: 0;">
        ...
        <a href="/servicos/especialista-cachos-bh">Especialista em Cachos BH</a>
        <a href="/servicos/inside-trp">Reconstrução Inside TRP</a>
        <a href="/imprensa">Imprensa e Mídia</a>
