...
// Dummy templates mapping for output info
const templates = {
  'd1': { subject: '{nome}, como tá o fio hoje?' },
  'd7': { subject: 'A semana mais importante do seu cabelo (e quase ninguém fala sobre isso)' },
  'd21': { subject: '3 semanas de corte novo. Agora vem a parte boa.' },
  'd35': { subject: 'Um mês de corte (e o segredo para ele durar mais)' },
  'd60': { subject: 'Falta 1 mês para a sua janela de manutenção' },
  'd90': { subject: 'Últimos dias do valor especial de manutenção, {nome}' },
  'aniversario': { subject: 'Seu aniversário tá chegando, {nome}' },
  'reativacao_5_meses': { subject: 'Seu cabelo tem memória, {nome}' }
};
    ...
    for (const daysAgo of windows) {
      ...
      if (snap.size > 0) {
        ...
        for (const bDoc of snap.docs) {
          ...
          else if (daysAgo === 50 && chem) { tplKey = 'd35'; subSwitchName = 'seqD35'; }
