        ...
        <div style={{
          ...
          <div style={{
            ...
            <h3 style={{
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>🛠️ Diagnóstico do App</span>
              <button 
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', background: '#f7fafc', padding: '16px', borderRadius: '10px', border: '1px solid #e2e8f0', marginBottom: '20px' }}>
              <div><strong>Versão Local:</strong> 20260605_v9</div>
              <div><strong>Modo Demo:</strong> {isDemoMode ? '✅ Sim (Offline/Mock)' : '❌ Não (Firebase Ativo)'}</div>
              <div><strong>Warning Conexão:</strong> {connectionWarning ? '⚠️ Sim' : '✅ Não'}</div>
              <div><strong>API Key Firebase:</strong> {import.meta.env.VITE_FIREBASE_API_KEY ? `${import.meta.env.VITE_FIREBASE_API_KEY.substring(0, 8)}...${import.meta.env.VITE_FIREBASE_API_KEY.substring(import.meta.env.VITE_FIREBASE_API_KEY.length - 4)}` : '❌ AUSENTE'}</div>
              <div><strong>Project ID Firebase:</strong> {import.meta.env.VITE_FIREBASE_PROJECT_ID || '❌ AUSENTE'}</div>
              <div><strong>Auth Inicializado:</strong> {auth ? '✅ Sim' : '❌ Não (Null)'}</div>
              <div><strong>Usuário Logado:</strong> {auth?.currentUser ? `✅ Sim (${auth.currentUser.email})` : '❌ Não'}</div>
              <div style={{ wordBreak: 'break-all' }}><strong>Último Erro Sync:</strong> {syncError ? `<span style="color:#e53e3e">${syncError}</span>` : 'Nenhum'}</div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <button
                onClick={async () => {
                  try {
                    ...
                    if (!user) throw new Error('Nenhum usuário autenticado no momento.');
                    const token = await user.getIdToken(true);
                    alert(`Token renovado com sucesso:\n${token.substring(0, 50)}...`);
                  } catch (e) {
                    alert(`Erro no token: ${e.message}`);
                  }
                }}
                style={{ 
                  background: '#3182ce', 
              ...
              <button
                onClick={() => {
                  ...
                  alert('Limpando cache do navegador/PWA. O aplicativo será recarregado agora.');
